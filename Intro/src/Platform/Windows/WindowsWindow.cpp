...
namespace Intro {
	...
	{
		ITR_CORE_ERROR("GLFW Error ({0}) : {1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& prop)
	{
		...
		ITR_CORE_INFO("Creating window {0} ({1}, {2})", prop.Title, prop.Width, prop.Height);

		if (!s_GLFWInitialized)
		{
			int success = glfwInit();
			ITR_CORE_ASSERT(success, "Could not intialize GLFW!");
		...
		ITR_ASSERT(status, "Failed to initailize Glad!");
